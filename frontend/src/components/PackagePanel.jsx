...
export default function PackagePanel({ tripId, components, onSwapped, onNext, loading }) {
  ...
  return (
    <div className="panel" style={{ padding: 24 }}>
      <h2 className="serif" style={{ fontSize: 20, fontWeight: 500, margin: "0 0 4px" }}>Your package</h2>
      <p className="dim" style={{ fontSize: 13, margin: "0 0 18px" }}>
        Swap anything with a "swap" tag — the total updates live and re-checks your budget.
      </p>

      {Object.entries(byDay).sort(([a], [b]) => a - b).map(([day, comps]) => (
        <div key={day} style={{ marginBottom: 16 }}>
          <div className="dim" style={{ fontSize: 11, textTransform: "uppercase", letterSpacing: "0.05em", marginBottom: 8 }}>Day {day}</div>
          {comps.map((c) => (
            <div key={c.component_id} style={{ marginBottom: 8 }}>
              ...
              {openFor === c.component_id && (
                <div style={{ padding: "8px 12px", background: "var(--paper-raised)", borderRadius: "var(--radius)", marginTop: 4 }}>
                  {altsLoading ? (
                    <div className="dim" style={{ fontSize: 12 }}>Loading alternatives…</div>
                  ) : alts.length === 0 ? (
                    <div className="dim" style={{ fontSize: 12 }}>No alternatives available for this item right now.</div>
                  ) : (
                    alts.map((a) => (
                      <button
                        key={a.component_id}
